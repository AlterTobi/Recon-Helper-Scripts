// ==UserScript==
// @name           RHS - maps open in
// @version        0.0.1-beta30.test.events
...
// @downloadURL    https://altertobi.github.io/Recon-Helper-Scripts/dev/rhs-openIn.user.js
// @updateURL      https://altertobi.github.io/Recon-Helper-Scripts/dev/rhs-openIn.meta.js
...
(function() {
  ...
  function addDropdownReview() {
    ...
    switch (pageData.type) {
      case "NEW":
        w.rhs.f.awaitElem("#check-duplicates-card > div.wf-review-card__body > div > div.mt-2.flex.justify-between.pb-1.space-x-4 > div:nth-child(1) > button")
          .then((elem) => {
      ...
      case "EDIT": {
        ...
        if (edit.isEdit) {
          ...
          mainButton.classList.add("rhs-marginTop");
  ...
  function showCaseLoaded() {
    const showcase = w.rhs.g.showcase();
  ...
  w.addEventListener("OPRReviewPageLoaded", () => {setTimeout(addDropdownReview, 100);});
  w.addEventListener("OPRNominationDetailLoaded", () => { clearTimeout(selNomTimerId); selNomTimerId = setTimeout(addDropdownNomination, 250);});
  w.addEventListener("OPRNominationListLoaded", () => { clearTimeout(loadNomTimerId); loadNomTimerId = setTimeout(NominationPageLoaded, 250);});
