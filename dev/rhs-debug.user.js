// ==UserScript==
// @name           RHS - Debug
// @version        0.0.6-beta24.test.events
...
// @downloadURL    https://altertobi.github.io/Recon-Helper-Scripts/dev/rhs-debug.user.js
// @updateURL      https://altertobi.github.io/Recon-Helper-Scripts/dev/rhs-debug.meta.js
...
(function() {
  ...
  const myCssId = "rhs-debugCSS";
  const myStyle = `.rhs-debug {
    position : absolute;
    top : 75px;
    right : 15px;
...
*/
  ...
  function showDebugBox(candidate, lskips) {
    ...
    if (!overlay) {
      ...
      overlay.setAttribute("class", "rhs-debug");
      ...
      switch(key) {
        ...
        case "t1": {
          if (0.5 === candidate[key]) {
            ...
            content += `<p class="rhs-red"><strong>${key}:</strong> ${candidate[key]}</p>`;
          }
          break;
        }
        default:
  ...
  function nominationDetail() {
    const lskips = ["supportingImageUrl", "imageUrl", "statement", "rejectReasons",
      ...
      "order", "day"];
    const candidate = w.rhs.g.nominationDetail();
    const poiId = w.rhs.g.nominationsList().find(obj => obj.id === candidate.id).poiData.id;
    if (poiId) {
  ...
  function addMail2ProfilePic() {
    ...
    const props = w.rhs.g.properties();
    const email = props.socialProfile.email;
    w.rhs.f.awaitElem(profileImageSelector).then(elem=>{
